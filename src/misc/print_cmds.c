...
void	print_cmd(t_excmd *cmd)
{
	...
	printf("[%p]   %-12s %s%p%s\n", cmd, "prototype",
		U_MAGENTA, cmd->proto, RESET);
	printf("[%p]   -\n[%p]   %-12s %s%d%s\n", cmd, cmd, "argc",
		B_YELLOW, cmd->argc, RESET);
	argv_len = 0;
	while (cmd->argv[argv_len] && argv_len < 1000)
		argv_len++;
	printf("[%p]   %-12s %s%p%s (%s%d%s detected)\n", cmd, "argv",
		B_YELLOW, cmd->argv, RESET, B_YELLOW, argv_len, RESET);
	if (argv_len)
	{
	...
	while (cmd->envp[envp_len] && envp_len < 1000)
		envp_len++;
	printf("[%p]   %-12s %s%p%s (%s%d%s detected)\n", cmd, "envp",
		U_MAGENTA, cmd->envp, RESET, B_YELLOW, envp_len, RESET);
	paths_len = 0;
	...
	if (cmd->pipe_open[1])
		...
		printf("%sCLOSED%s]", B_RED, RESET);
	printf("[%s%d%s, %s%d%s]\n", B_YELLOW, cmd->pipe[0],
		RESET, B_YELLOW, cmd->pipe[1], RESET);
	printf("[%p]   -\n[%p]   %s%p%s <= %s%p%s => %s%p%s\n", cmd, cmd, WHITE,
		cmd->prev, RESET, U_MAGENTA, cmd, RESET, WHITE, cmd->next, RESET);
}
