...
override	SRC_MISC		:=	show_prompt signals signals_handlers build_themes get_random_chars
...
override	SRC_UTILS		:=	$(addprefix parsing/, count_arg_words free_str_return_null ft_add_char ft_split_parser ft_str_join_free ft_strcmp get_first_word handle_is_redir_token incr_on_alnum is_redir print_t_token_list skip_spaces token_lstsize) \
...
CFLAGS		:=	-Wall -Wextra -MMD -MP -g3 -D PROJECT_NAME=\"$(NAME)\" #Remetttre Werror
...
eh:
	...
	$(MAKE) bonus
	clear
	env -i valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes --show-mismatched-frees=yes --track-fds=yes --trace-children=yes --suppressions=/home/ehosta/Documents/42-minishell/.valgrind_suppress.txt ./$(NAME)
