...
typedef struct s_excmd
{
	...
	/**
		...
		* Terminates with a NULL element. If empty,
			only contains a NULL element.
		*/
	char				**envp;
	/**
	...
	char				*raw;
	/**
		* The PATH environment variable splitted into a string list,
			used for the
		* execve loop. If empty, only contains a NULL element.
		*/
	char				**paths;
	/**
		* If it includes an Here Document,
			this boolean is set to true. Otherwise
		* it is set to false.
		*/
	t_bool				has_heredoc;
	/**
	...
	/**
		...
		* memory,
			and then put into a pipe's output file descriptor when it has to
