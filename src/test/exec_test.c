...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/14 15:26:32 by ehosta            #+#    #+#             */
/*   Updated: 2025/04/14 15:28:16 by ehosta           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	const t_excmd	*cat1 = create_cmd("cat", &minishell->env);
	const t_excmd	*cat2 = create_cmd("cat", &minishell->env);
	const t_excmd	*ls = create_cmd("ls", &minishell->env);
...
{
	const t_excmd	*cat = create_cmd("cat", &minishell->env);
	const t_excmd	*head = create_cmd("head", &minishell->env);
...
{
	const t_excmd	*cat = create_cmd("env", &minishell->env);
...
static t_excmd	*_complete_test_1(t_minishell *minishell)
{
	...
	cat1 = create_cmd("cat", &minishell->env);
