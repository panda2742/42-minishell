...
/*   Updated: 2025/04/23 14:13:25 by abonifac         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_head_and_last(t_token **new_last, t_token **new_head,
							t_token *new_token)
{
	if (!*new_head)
	{
	...
	while (expanded[i])
	{
		if (ft_isspace(expanded[i]))
		{
			if ((*current)[0] != '\0')
			{
				...
				update_head_and_last(&(*new_list)->new_last,
					&(*new_list)->new_head, new_token);
				free(*current);
				*current = ft_strdup("");
			}
...
char	*handle_quote_none(t_fragment *frag, t_minishell *mini,
							t_word_split *new_list, t_token *token)
{
	char	*expanded;
...
char	*handle_other_quotes(t_fragment *frag, t_minishell *mini,
								char *current)
{
	char	*expanded;
